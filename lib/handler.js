...
const InternalError = [500, 502, 503, 504];
...
class Handler {
	...
	_handlePayload(payload, callback) {
		...
		_this.request(payload.request_object, function (err, req, body) {
			if (err) {
				...
				if (InternalError.indexOf(body.meta.code) !== -1) {
					_this._retryPayload('InternalError', payload, body, callback);
     ...
     *
	 * @param type {string} - 'InternalError' for return API Error / 'ETIMEDOUT' for return ConnectionTimeout
     * @param payload {object} - The payload object to create the request
	 * @param body {object} - If body is defined, it is an InternalError
     * @param callback {function}
     */
