...
class AftershipError {
	...
	static getApiError(response_body, retry_count) {
		let error = new Error();
		error.type = _.get(response_body, 'meta.type');
		error.message = _.get(response_body, 'meta.message');
		error.code = _.get(response_body, 'meta.code');
		error.data = _.get(response_body, 'data');;
